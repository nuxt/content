...
const operators: Record<string, QueryMatchOperator> = {
...
plugins.forEach((plugin: QueryPlugin) => {
  Object.assign(operators, plugin.operators)
})

/**
 * Math item with condition
 */
export function match(item: any, conditions: any): boolean {
  return Object.keys(conditions || {}).every(key => {
    ...
    if (typeof condition === 'object') return match(value, condition)
