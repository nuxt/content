...
export default eventHandler(async (event) => {
  ...
  if (url.pathname.endsWith('/sql_dump.enc')) {
    ...
    const prebuilt = await storage.getItem<string>(encKey)
    if (prebuilt) {
    ...
    const storedPlain = await storage.getItem<string>(plainKey)
    const gzBase64 = storedPlain ?? await (async () => {
      const asset = await fetchFromAssets(event, plaintextAssetCandidates(collection))
  ...
  const plainKey = `build:content:raw:dump.${collection}.sql`
  const storedPlain = await storage.getItem<string>(plainKey)
  const plain = storedPlain ?? await (async () => {
    const asset = await fetchFromAssets(event, plaintextAssetCandidates(collection))
    return asset ? asset.body : null
  })()
  if (plain) {
