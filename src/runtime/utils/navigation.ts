...
export function findPageBreadcrumb(navigation?: NavigationItem[], path?: string | undefined | null, options?: FindPageBreadcrumbOptions): NavigationItem[] {
  if (!navigation?.length || !path) {
    return []
  }

  return navigation.reduce((breadcrumb: NavigationItem[], link: NavigationItem) => {
...
export function findPageChildren(navigation?: NavigationItem[], path?: string | undefined | null, options?: FindPageOptions): NavigationItem[] {
  if (!navigation?.length || !path) {
    return []
  }

  return navigation.reduce((children: NavigationItem[], link: NavigationItem) => {
    if (link.children) {
      if (path === link.path) {
...
export function findPageSiblings(navigation?: NavigationItem[], path?: string | undefined | null, options?: FindPageOptions): NavigationItem[] {
  if (!navigation?.length || !path) {
    return []
  }
...
export function findPageHeadline(navigation?: NavigationItem[], path?: string | undefined | null, options?: FindPageOptions): string | undefined {
  if (!navigation?.length || !path) {
    return
  }
