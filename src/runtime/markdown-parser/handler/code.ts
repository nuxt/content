...
const prismHighlighter = (
   ...
   * Set pre as a child
   */
  childs.push(h(node, 'pre', props, [h(node, 'code', { ignoreMap: 'true' }, [u('raw', code)])]))

  return h(node.position, 'code', {}, childs)
}

const toAst = (h: any, node: any) => (highlighted: any) => {
  if (typeof highlighted === 'string') {
    return h(node, 'code', { ignoreMap: 'true' }, [u('raw', highlighted)])
  }
  return highlighted
}
