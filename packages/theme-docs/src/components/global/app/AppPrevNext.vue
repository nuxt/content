<template>
  ...
  >
    <NuxtLink
      v-if="prev"
      :to="toLink(prev.slug)"
      class="text-malachite-500 font-bold hover:underline flex items-center p-2 pl-0"
    >
    ...
    <NuxtLink
      v-if="next"
      :to="toLink(next.slug)"
      class="text-malachite-500 font-bold hover:underline flex items-center p-2 pr-0"
    >
      {{ next.title }}
