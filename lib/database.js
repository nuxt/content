...
class Database extends Hookable {
  constructor (options) {
    ...
    // User Parsers
    this.userParser = options.userParser
    this.userExtensions = Object.keys(options.userParser)
  ...
  async parseFile (path) {
    ...
    if (!EXTENSIONS.includes(extension) && !this.userExtensions.includes(extension)) {
      return
    }
