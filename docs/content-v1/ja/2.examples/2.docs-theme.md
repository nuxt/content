...
このテーマは国際化に[nuxt-i18n](https://i18n.nuxtjs.org/)を使用します。そのデフォルトは`en`ロケールであるため、`content/en/`サブディレクトリを作成する必要があります。その後、Markdownファイルの書き込みを開始できます
...
- `logo`
  - Type: `string` | `object`
  - *プロジェクトのロゴ。[color mode](https://v2.color-mode.nuxtjs.org/)ごとにロゴを設定する`object`にもできます*
- `github`
  - Type: `string`
...
Check out an info alert with a [link](/themes/docs).<!--TODO: fix link?-->
