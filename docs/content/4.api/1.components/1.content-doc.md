...
An explicit path can be passed to the component with the `path`{lang=ts} props, if none is provided, the `$route.path`{lang=ts} will be used.
...
The `not-found`{lang=ts} slot can be used when not document is found:
