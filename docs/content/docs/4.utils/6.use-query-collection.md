...
The returned chainable builder mirrors `queryCollection`, but its terminal methods (`all`, `first`, `count`) return an `AsyncData` result instead of a promise.

The optional generic `R` overrides the return type. When omitted, the collection's generated type is used.

::warning
Because TypeScript has no partial type-argument inference, providing the `R` override (for example `useQueryCollection<CardItemType>('docs')`) widens the inferred collection type. Field-typed methods such as `.where()`, `.order()`, and `.select()` then fall back to the keys common to all collections. Pass `R` only when you do not rely on per-field type checking for that query, or cast the result instead.
::
