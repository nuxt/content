...
Terminal methods execute the query and return [`AsyncData`](https://nuxt.com/docs/4.x/api/composables/use-async-data#return-values), which exposes `data`, `pending`, `error`, `status`, `refresh()`, `execute()`, and `clear()` alongside the result:
