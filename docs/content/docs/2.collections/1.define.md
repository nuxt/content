...
type Collection = {
  ...
  // Enable multi-language support. `true` reads `locales` and `defaultLocale`
  // from the `@nuxtjs/i18n` module configuration. An object opts in
  // explicitly without requiring the module.
  i18n?: true | { locales: string[], defaultLocale: string }
}
