...
function getCollectionFromPath(pathname: string): string | null {
  const m = pathname.match(/\/(?:api\/)?__nuxt_content\/([^/]+)/)
  return m ? m[1] : null
}

function isKeyEndpoint(pathname: string): boolean {
  return /\/(?:api\/)?__nuxt_content\/[^/]+\/key\/?$/.test(pathname)
}
...
- The /api/__nuxt_content/:collection/key endpoint is invoked after this middleware.
...
- If you **don’t hand out a key** from `/api/__nuxt_content/:collection/key`, the client cannot decrypt that collection’s dump.  
...
- Since the **checksum** of the dump is included in the derivation, a new build with updated content produces a new key.  
  Old keys won’t work with updated dumps.
