...
- The `<ContentRichSlot />`{lang="html"} component to accept formatted text.
...
The `<ContentRichSlot />`{lang="html"} component is auto-imported by Nuxt Content. It acts as a special slot that accepts rich text rendered by Markdown.
...
The `<ContentRichSlot />` component can act as a named slot with the `use` property:

```html
<ContentRichSlot :use="$slots.description" unwrap="p">
```

## Inline components

Inline components are components without slots or `<ContentRichSlot />`.
