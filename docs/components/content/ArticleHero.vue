<template>
  <div class="py-10 sm:py-20">
    <div class="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8 flex flex-col gap-y-8 items-center justify-center text-center">
      ...
      <h1 class="font-bold text-gray-900 text-5xl">
        {{ page.title }}
      </h1>
      <p class="text-gray-700 font-medium max-w-5xl">
        {{ page.description }}
      </p>
      <ul class="flex flex-wrap items-center justify-center gap-4">
        <li v-for="author in page.authors" :key="author.name" class="group">
          <a :href="author.link" alt="twitter account" class="flex gap-x-2 group-hover:bg-gray-100 p-2 rounded-md transition-colors duration-300">
